...
static inline RepBuffer encodeWholeNumber(float x)
{
    int isInteger = floorf(x) == x;
...
static inline int requiresEngineerNotation(float value)
{
...
uint encodeNumber(float value)
{
    ...
    RepBuffer buf = requiresEngineerNotation(value)
                    ? encodeEngNotation(value)
                    : encodeWholeNumber(value);
