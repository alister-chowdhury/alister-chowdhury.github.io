...
function createGraphicsProgram(GL, vertexShader, fragmentShader)
{
    if(!GL)
    {
        return null;
...
const _DRAW_FULL_SCREEN_VS_SRC_SYNC = loadShaderSource("draw_full_screen.vert");
const _VOID_AND_CLUSTER_INIT_FS_SRC_SYNC = loadShaderSource("void_and_cluster_init.frag");
const _VOID_AND_CLUSTER_REDUCE_INIT_FS_SRC_SYNC = loadShaderSource("void_and_cluster_reduce_init.frag");
const _VOID_AND_CLUSTER_REDUCE_ITER_FS_SRC_SYNC = loadShaderSource("void_and_cluster_reduce_iter.frag");
const _VOID_AND_CLUSTER_UPDATE_FS_SRC_SYNC = loadShaderSource("void_and_cluster_update.frag");
const _VOID_AND_CLUSTER_PARTIAL_UPDATE_VS_SRC_SYNC = loadShaderSource("void_and_cluster_partial_update.vert");
const _VIS_BLUENOISE_TILED_FS_SRC_SYNC = loadShaderSource("vis_bluenoise_tiled.frag");
const _DRAW_FULL_SCREEN_WITH_UVS_VS_SRC_SYNC = loadShaderSource("draw_full_screen_with_uvs.vert");
const _VIS_BLUENOISE_SCALED_FS_SRC_SYNC = loadShaderSource("vis_bluenoise_scaled.frag");
const _EXPORT_FIRST_CHANNEL_FS_SRC_SYNC = loadShaderSource("export_first_channel.frag");
const _BC4_COMPRESS_FS_SRC_SYNC = loadShaderSource("bc4_compress.frag");


const _DRAW_FULL_SCREEN_VS_SRC = await _DRAW_FULL_SCREEN_VS_SRC_SYNC;
const _VOID_AND_CLUSTER_INIT_FS_SRC = await _VOID_AND_CLUSTER_INIT_FS_SRC_SYNC;
const _VOID_AND_CLUSTER_REDUCE_INIT_FS_SRC = await _VOID_AND_CLUSTER_REDUCE_INIT_FS_SRC_SYNC;
const _VOID_AND_CLUSTER_REDUCE_ITER_FS_SRC = await _VOID_AND_CLUSTER_REDUCE_ITER_FS_SRC_SYNC;
const _VOID_AND_CLUSTER_UPDATE_FS_SRC = await _VOID_AND_CLUSTER_UPDATE_FS_SRC_SYNC;
const _VOID_AND_CLUSTER_PARTIAL_UPDATE_VS_SRC = await _VOID_AND_CLUSTER_PARTIAL_UPDATE_VS_SRC_SYNC;
const _VIS_BLUENOISE_TILED_FS_SRC = await _VIS_BLUENOISE_TILED_FS_SRC_SYNC;
const _DRAW_FULL_SCREEN_WITH_UVS_VS_SRC = await _DRAW_FULL_SCREEN_WITH_UVS_VS_SRC_SYNC;
const _VIS_BLUENOISE_SCALED_FS_SRC = await _VIS_BLUENOISE_SCALED_FS_SRC_SYNC;
const _EXPORT_FIRST_CHANNEL_FS_SRC = await _EXPORT_FIRST_CHANNEL_FS_SRC_SYNC;
const _BC4_COMPRESS_FS_SRC = await _BC4_COMPRESS_FS_SRC_SYNC;
...
class GraphicsPSOs
{
    constructor(GL)
    {
        const drawFullScreenVS = createShader(GL, _DRAW_FULL_SCREEN_VS_SRC, GL.VERTEX_SHADER);
        const voidAndClusterInitFS = createShader(GL, _VOID_AND_CLUSTER_INIT_FS_SRC, GL.FRAGMENT_SHADER);
        const voidAndClusterReduceInitFS = createShader(GL, _VOID_AND_CLUSTER_REDUCE_INIT_FS_SRC, GL.FRAGMENT_SHADER);
        const voidAndClusterReduceIterFS = createShader(GL, _VOID_AND_CLUSTER_REDUCE_ITER_FS_SRC, GL.FRAGMENT_SHADER);
        const voidAndClusterUpdateFS = createShader(GL, _VOID_AND_CLUSTER_UPDATE_FS_SRC, GL.FRAGMENT_SHADER);
        const voidAndClusterPartialUpdateVS = createShader(GL, _VOID_AND_CLUSTER_PARTIAL_UPDATE_VS_SRC, GL.VERTEX_SHADER);
        const visBluenoiseTiledFS = createShader(GL, _VIS_BLUENOISE_TILED_FS_SRC, GL.FRAGMENT_SHADER);
        const drawFullScreenWithUvsVS = createShader(GL, _DRAW_FULL_SCREEN_WITH_UVS_VS_SRC, GL.VERTEX_SHADER);
        const visBluenoiseScaledFS = createShader(GL, _VIS_BLUENOISE_SCALED_FS_SRC, GL.FRAGMENT_SHADER);
        const exportFirstChannelFS = createShader(GL, _EXPORT_FIRST_CHANNEL_FS_SRC, GL.FRAGMENT_SHADER);
        const bc4CompressFS = createShader(GL, _BC4_COMPRESS_FS_SRC, GL.FRAGMENT_SHADER);
...
class BluenoiseRenderContext
{
    ...
    constructor(GL, PSOs, width, height, sigma, updateAccuracy)
    {
        ...
        if((updateAccuracy > 0) && (updateAccuracy < 1))
        {
            // We can calculate the total energy distributed over one dimenson via:
            //
            // ∞
            // ∫ e^(-x * a^-1) dx = a
            // 0
            ...
            //
            // (Hopefully this math is legit)
            updateSpan = Math.ceil(-Math.log(1 - updateAccuracy) * sigma * sigma);
        }
...
class CPUTimerPool
{
    ...
    getNumberOfIterations()
    {
        ...
        const timeDelta = this.startTime - Date.now();
